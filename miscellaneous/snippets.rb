...
snippet '<?echo"<pre>";var_export($arResult);echo"</pre>";?>' do |s|
  s.trigger = 'epr'
  s.expansion = 'echo\'<pre> $arResult: \';var_export($arResult);echo\'</pre>\';'
end

snippet 'file_put_contents($arResult)";' do |s|
  s.trigger = 'fpc'
  s.expansion = 'file_put_contents(\$_SERVER[\'DOCUMENT_ROOT\'] . \'/saintiago-log.txt\', date("d.m.Y H:i:s -- ") . \'$arResult: \' . var_export($arResult, true) . "\n", FILE_APPEND);'
end

snippet 'CIBlockElement::GetList()' do |s|
  s.trigger = 'getlist'
  s.expansion = 
'
\\$arSort = array("sort" => "asc");
\\$arFilter = array("IBLOCK_ID" => ${1:iblock_id}, "ID" => ${2:id});
\\$arSelect = array(${3:fields});
\\$obElement = CIBlockElement::GetList(\\$arSort,\\$arFilter,false,false,\\$arSelect);
...
snippet '$_SERVER[\'REMOTE_ADDR\']' do |s|
  s.trigger = 'remote'
  s.expansion = 
'
if(\$_SERVER[\'REMOTE_ADDR\'] == \'92.255.243.152\')
{
  $var
}
'
end
snippet 'CPHPCACHE' do |s|
  s.trigger = 'cache'
  s.expansion = 
'
\\$cache = new CPHPCache();
\\$cache_time = 3600;
\\$cache_id = "${1:cacheID}";
\\$cache_path = "/${1:cacheID}/";
if (\\$cache_time > 0 && \\$cache->InitCache(\\$cache_time, \\$cache_id, \\$cache_path))
{
   \\$res = \\$cache->GetVars();
   if (isset(\\$res["${2:VARIABLE}"]))
      ${3:variable} = \\$res["${2:VARIABLE}"];
}
if (!isset(${3:variable}))
{
   
  //poluchaem dannye iz bazy v variable

   if (\\$cache_time > 0)
   {
         \\$cache->StartDataCache(\\$cache_time, \\$cache_id, \\$cache_path);
         \\$cache->EndDataCache(array("${2:VARIABLE}" => ${3:variable}));
   }
}
'
...
snippet 'CFile::ResizeImageGet' do |s|
  s.trigger = 'resize'
  s.expansion = '\\$resizedPhoto = CFile::ResizeImageGet(${1:source},array("width" => ${2:width},"height" => ${3:height}), ${4:resize_method},true);'
end

snippet 'BX_RESIZE_IMAGE_EXACT' do |s|
  s.trigger = 'exact'
  s.expansion = 'BX_RESIZE_IMAGE_EXACT'
end

snippet 'BX_RESIZE_IMAGE_PROPORTIONAL' do |s|
  s.trigger = 'prop'
  s.expansion = 'BX_RESIZE_IMAGE_PROPORTIONAL'
end

snippet 'IsAdmin' do |s|
  s.trigger = 'isa'
  s.expansion = 'if (\\$USER->IsAdmin()) {$var}'
end

snippet 'prolog_before' do |s|
  s.trigger = 'prolog'
  s.expansion = 'require(\\$_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_before.php");'
end

snippet 'IncludeModule("iblock")' do |s|
  s.trigger = 'iblock'
  s.expansion = 'if (!CModule::IncludeModule("iblock")) die("Error including iblock module");'
end

snippet 'ParseComponentPath' do |s|
  s.trigger = 'ParseComp'
  s.expansion = 
'
\\$sefFolder = "${1:sef_folder}";
\\$arUrlTemplates = array(
  "element" => "${2:url_template}"
);
\\$arVariables = array();
\\$page = 
    CComponentEngine::ParseComponentPath(\\$sefFolder, 
